...
export function getRabbitColors(colorName: string): RabbitColors {
  ...
  if (resolvedColor.includes('beige') || resolvedColor.includes('gold')) {
  ...
  if (resolvedColor.includes('black') || resolvedColor.includes('grey') || resolvedColor.includes('gray')) {
    return { body: "#d1d5db", belly: "#f3f4f6", ears: "#9ca3af", accent: "#4b5563" };
  }
...
export function TeddyRabbitSVG({ colors, className, size }: { colors: RabbitColors; className?: string; size?: number }) {
  return (
    ...
    >
      {/* ── TAIL ── */}
      <circle cx="246" cy="226" r="16" fill={colors.ears} opacity="0.9" />
      <circle cx="246" cy="226" r="10" fill={colors.body} opacity="0.7" />

      {/* ── LONG RABBIT EARS ── */}
      {/* Left Ear */}
      <ellipse cx="120" cy="54" rx="16" ry="46" fill={colors.body} />
      <ellipse cx="120" cy="58" rx="9"  ry="32" fill={colors.ears} opacity="0.8" />
      {/* Right Ear */}
      <ellipse cx="196" cy="54" rx="16" ry="46" fill={colors.body} />
      <ellipse cx="196" cy="58" rx="9"  ry="32" fill={colors.ears} opacity="0.8" />
      ...
      <ellipse cx="158" cy="133" rx="8"  ry="6.5" fill={colors.ears} />
      {/* smile */}
      <path d="M 144 148 Q 158 160 172 148" stroke={colors.accent} strokeWidth="2.5" strokeLinecap="round" fill="none" opacity="0.75" />
      {/* whiskers */}
      <line x1="120" y1="144" x2="100" y2="140" stroke={colors.accent} strokeWidth="1.2" opacity="0.4" />
      <line x1="120" y1="150" x2="98"  y2="152" stroke={colors.accent} strokeWidth="1.2" opacity="0.4" />
      <line x1="196" y1="144" x2="216" y2="140" stroke={colors.accent} strokeWidth="1.2" opacity="0.4" />
      <line x1="196" y1="150" x2="218" y2="152" stroke={colors.accent} strokeWidth="1.2" opacity="0.4" />
